...
void configure_rx_blocks(mimorph::mimorph& radio, bool bw, uint8_t rx_split){
    ...
    //cfo correction
    radio.control->set_rx_cfo_correction_param(radio_config->bw,false,1);//poner defines con los scaling //el escalado no funciona si el bloque no esta habilitado
...
int main() {
    ...
    auto* radio_config=radio.control->get_radio_config();
    //std::vector<mimorph::slot_str> rx_data;
    mimorph::slot_str rx_data(num_of_rx_bytes,radio_config->ofdm.num_sc*4);

    //reserve space for X slots
/*    int burst_size=10;
    for (int i = 0; i < burst_size; ++i) {
        rx_data.emplace_back(num_of_rx_bytes,radio_config->ofdm.num_sc*4);
    }*/
