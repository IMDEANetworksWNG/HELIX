...
#include "tools.h"
#include "xiicps.h"
#define PRINTF printf /* Print macro for BM */
XIicPs Iic1;
...
int clockCFG_init(u32 ConfigId){

	XRFClk_Init(XPAR_AXI_GPIO_SPI_MUX_BASEADDR);

	/* Set config with ID */
	printf("\n\nSet config from ID on LMK");
	if (XST_FAILURE ==
	    XRFClk_SetConfigOnOneChipFromConfigId(RFCLK_LMK, ConfigId)) {
